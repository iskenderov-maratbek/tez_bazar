...
class ArchiveList extends ConsumerStatefulWidget {
  const ArchiveList({super.key});

  @override
  ConsumerState<ArchiveList> createState() => _ArchiveListState();
}

class _ArchiveListState extends ConsumerState<ArchiveList> {
  ...
  void initState() {
    ...
    ref
        .read(archiveProductsProvider.notifier)
  ...
  Widget build(BuildContext context) {
    ...
    final product = ref.watch(archiveProductsProvider);
    return PageBuilder(
      onRefresh: () async {
        logInfo('archiveList onRefresh');
        ref.read(archiveProductsProvider.notifier).clear();
        ref
            .read(archiveProductsProvider.notifier)
            .getUserProducts(ref.read(authServiceProvider)!.id);
      },
